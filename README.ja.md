...
- **8フェーズの変換パイプライン** (Phase 1.5 含む) — 段階的な自動修復つき
...
`convert` は建物ごとに以下の8フェーズ (Phase 1.5 を含む) を実行します:
...
result = search_buildings_by_address(
    "東京都千代田区霞が関3-2-1",
    search_mode="hybrid",  # "distance", "name", "hybrid"
    limit=10,
)
...
- メッシュ→市区町村コードのオフラインマッピング (`plateau.api_client` の全国JSON + `plateau.mesh_mapping` の東京フォールバック)
